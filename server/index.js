...
app.get('/yahoo/team', async (req, res) => {
	...
	if (!accessToken) return res.status(400).send('Missing access_token');
	try {
		const url = `https://fantasysports.yahooapis.com/fantasy/v2/team/461.l.146891.t.${req.query.teamId}?format=json`;
		const response = await axios.get(url, {
			headers: {
				Authorization: `Bearer ${accessToken}`,
			},
...
app.get('/yahoo/team/stats', async (req, res) => {

	const accessToken = req.query.access_token;
	if (!accessToken) return res.status(400).send('Missing access_token');
	try {
		const url = `https://fantasysports.yahooapis.com/fantasy/v2/team/461.l.146891.t.${req.query.teamId}/stats?format=json`;
		const response = await axios.get(url, {
			headers: {
				Authorization: `Bearer ${accessToken}`,
			},
...
app.get('/yahoo/team/matchups', async (req, res) => {
