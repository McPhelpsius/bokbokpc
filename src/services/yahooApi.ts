...
export async function fetchLeagueData(): Promise<any> {
  ...
  const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/yahoo/league?access_token=${token}`);
  ...
  if (!res.ok) {
    if (res.status === 401) {
      ...
      if (newToken) {
        ...
        const retryRes = await fetch(`${import.meta.env.VITE_API_BASE_URL}/yahoo/league?access_token=${newToken}`);
        if (retryRes.ok) {
