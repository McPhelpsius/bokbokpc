...
export const redirectToYahooAuth = () => {
  window.location.href = `0.0.0.0:8080/auth/yahoo`;
};
...
export const refreshAccessToken = async (): Promise<string | null> => {
  ...
  try {
    const response = await fetch(`0.0.0.0:8080/auth/yahoo/refresh`, {
      method: 'POST',
      headers: {
...
export async function fetchLeagueData(): Promise<any> {
  ...
  const res = await fetch(`0.0.0.0:8080/yahoo/league?access_token=${token}`);
  ...
  if (!res.ok) {
    if (res.status === 401) {
      ...
      if (newToken) {
        ...
        const retryRes = await fetch(`0.0.0.0:8080/yahoo/league?access_token=${newToken}`);
        if (retryRes.ok) {
